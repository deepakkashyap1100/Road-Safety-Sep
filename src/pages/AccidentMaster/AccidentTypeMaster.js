...
const AccidentTypeMaster = () => {
    const [isLoading, setIsLoading] = useState(false);
    const [typeOfAccidentData, setTypeOfAccidentData] = useState([]);
    ...
    const columns = [
        ...
        },
        {
            ...
            filterable: false,
            disableColumnMenu: true, headerClassName: "blue-header", renderCell: (params) => (
                <>
                    <div
    ...
    //=============== function to get type of accident data ===============\\
    const __getTypeofAccidentData = async () => {
        try {
            setIsLoading(true);
            let data = {
                Category: "Type",
            ...
            if (res.length > 0) {
                setTypeOfAccidentData(res);
            } else {
                toast.error("Failed to fetch Type of Accident data");
            }
        } catch (error) {
            toast.error(error?.message || error?.response?.data?.message || "Something went wrong");
        } finally {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-800">Accident Type Master</h1>
            </div>
...
export default AccidentTypeMaster